#!/usr/bin/env bash
# Launch mujoco_viewer_synthetic_idle_encoder_decoder.py through `mjpython` (required for the
...
exec env \
    ...
    "${MJPYTHON}" "${SCRIPT_DIR}/mujoco_viewer_synthetic_idle_encoder_decoder.py" "$@"
